...
| 操作履歴 | 1件のagent runでAIが実行した再現可能なブラウザ操作を、URL、操作内容、操作位置および結果とともに保存した記録。興味推定用の行動履歴とは分離する。 |
| リプレイ | 保存済み操作履歴をcomputer-useモデルの再推論なしで同じ隔離ブラウザへ順次適用する処理。 |
...
- 操作履歴の収集状態を行動履歴とは独立して確認・変更でき、保存済みrunの一覧、詳細、削除およびリプレイを実行できる。
- 操作履歴収集の初期値は有効とし、無効化しても保存済み履歴の閲覧、リプレイ、削除は継続できる。
...
| FR-BR-008 | ブラウザ画像取得に10秒のタイムアウトを設定する。タイムアウト、ページ終了またはブラウザクラッシュを検出した場合はブラウザを再生成し、画像取得を1回再試行する。復旧中は画面状態を明示する。 |
...
ユーザーは要求ごとに通常モードまたはタスクモードを選択できる。通常モードは従来どおり要求全体を1回のagent runとして処理する。タスクモードは、最初に複合要求からUI構造に依存しないゴール制約だけを抽出する。具体的な操作は固定ステップ列として先に生成せず、現在画面を観測するたびに、次に解決する1つの意味的なサブタスクだけを生成する。各サブタスクの実行結果を画面上の明示的な状態で検証し、全ゴール制約を再評価してから次のサブタスクを計画する。未達、画面変更、選択解除を検出した場合は最新画面から再計画し、全体の成功条件を最終確認するまで完了として扱わない。停止、手動介入、承認拒否時は現在のサブタスクと未実行状態を破棄する。

Goal Plannerは最終成功条件を構造化制約として生成する。Next-Step Plannerは現在画面、ゴール制約の検証結果、完了済みサブタスクおよび直近の失敗を入力として、次の1サブタスクとその事後条件だけを生成する。1サブタスクは1クリックではなく「見積もり画面へ進む」「F SPORTを選択済みにする」のような1目的を単位とする。未観測の後続UIを推測したり、複数カテゴリの操作UIが同時に表示されることを前提にしてはならない。構造化制約は一意なID、検証対象の種別とラベル、演算子、期待値、使用可能な証拠種別、説明を持つ。対象種別はページ、URL、選択状態、値、集合を、演算子は表示・非表示、選択・非選択、一致、包含、選択可能項目の全選択、件数一致、上限・下限を扱う。包含条件、除外条件、数量条件は別々の制約として表現する。選択または集合に対する一時的なUI可視性を成功条件としてはならず、操作後も選択済み要約や構成内容で確認できる事後条件を使用する。

VerifierはPlannerおよび操作担当モデルから独立し、最新スクリーンショット、ページ本文、URLのうち制約で指定された証拠だけを使って、制約IDごとに合否と根拠を返す。操作担当モデルの成功自己申告は証拠として扱わない。結果が欠落した制約、明示的な証拠がない制約、1件でも不合格の制約がある場合はサブタスクを完了として扱わない。
...
| FR-AG-004 | 1回のComputer Use実行チャンクに最大ステップ数と総タイムアウトを設定できる。初期値は20ステップとし、HTTPタイムアウトより前に画面とタスク状態を保存して次のチャンクへ継続する。チャンク境界をagent runの完了として扱わない。 |
...
| FR-AG-009 | タスクモードでは複合要求をUI非依存のゴール制約へ変換し、現在画面ごとに次の1サブタスクだけを生成する。サブタスクの検証後に全ゴールを再評価し、未達なら最新画面から次を計画する。通常モードの動作は維持する。 |
| FR-AG-010 | computer-useモデルが操作を終了しても、その終了報告または画面状態が成功条件の未達を示す場合は完了として扱わない。同一サブタスクを限定再試行し、解消できない場合はその固定サブタスクを破棄して現在画面から別の1手を再計画する。再計画上限でも解消できない場合は確認できた不足条件を未完了結果としてユーザーへ返す。 |
| FR-AG-011 | Goal Plannerは包含、除外、数量、完全選択、上限・下限を、対象、演算子、期待値および証拠種別を持つ構造化ゴール制約として生成する。特定カテゴリの完全選択では、ユーザーが別途明示した項目を除き、要求対象外の任意追加項目が0件である制約も生成する。Next-Step Plannerは未観測UIや複数UIの同時表示を仮定せず、1目的と操作後も確認可能な事後条件だけを生成する。特定の車種、装備名または画面固有の判定ロジックをタスク実行基盤へ埋め込まない。 |
| FR-AG-012 | Verifierは最新画面の証拠を構造化制約ごとに独立判定する。制約結果の欠落、証拠不足または不合格が1件でもある場合は完了を拒否し、限定再試行または未完了報告を行う。承認後の再開結果にも同じ検証を適用する。 |
| FR-AG-013 | 処理ログにはGoal Plannerのゴール制約、Next-Step Plannerの現在サブタスクと事後条件、Verifierの制約別判定と根拠、再計画理由を記録し、UI上で詳細を展開して確認できる。 |
| FR-AG-014 | 同一画面に対するスクリーンショット、ページ本文、URL、browser revisionを1つの観測として取得し、サブタスク制約と全ゴール制約を1回のVerifier呼び出しで判定する。画面revisionが変化した場合は観測を再利用しない。 |
| FR-AG-015 | Goal Planner、Next-Step Planner、Verifier、Computer Useについて、モデル名、モデル呼び出し回数、モデル待ち時間および画面取得時間を、プロンプト本文や画像本体を含めず処理ログへ記録する。 |
...
| ID | 要件 |
| --- | --- |
| FR-RP-001 | AIの1件のagent runを1件の操作履歴として保存し、元の要求、実行モード、開始・終了日時、開始・終了URL、結果、操作数を関連付ける。手動操作は操作履歴へ含めない。 |
| FR-RP-002 | 各操作には順序、操作種別、操作前後の正規化URL、画面上の座標、対象要素のタグ・表示名・リンク、操作前後のbrowser revision、実行時刻を保存する。加えて、要素中心の再生のため、role、アクセシブル名、test id、id、name、リンク、テキスト、同名要素中の序数、ダイアログ内かを含むロケータ記述子と、記録スキーマ版を保存する。 |
| FR-RP-003 | 文字入力値を操作履歴、API応答、処理ログへ保存しない。実際に値を入力した文字入力を含むrunはその事実だけを記録して全体をリプレイ不可とし、部分リプレイしない。空文字のみの入力は文字入力とみなさずリプレイ可否へ影響させない。値を入力した場合は、値を含めず入力欄ラベルのみを処理ログへ記録する。 |
| FR-RP-004 | 操作履歴収集は行動履歴収集と独立した設定としてUIからON/OFFでき、初期値をONとする。OFF中に開始したrunは保存しない。 |
| FR-RP-005 | run実行中に操作履歴収集をOFFにした場合、そのrunの保存済み部分を直ちに破棄する。同じrun中に再度ONにしても記録を再開せず、次のrunから有効にする。完了済み履歴は削除しない。 |
| FR-RP-006 | ユーザーは履歴一覧からrunを明示的に選択してリプレイを開始できる。要求との自動照合または自動リプレイは行わない。 |
| FR-RP-007 | リプレイは保存済み操作を順番に隔離ブラウザへ直接適用し、通常経路ではcomputer-useモデルを呼び出さない。リプレイ自身を新しい操作履歴として重複保存しない。 |
| FR-RP-008 | クリック系操作はロケータ記述子（test id、id、role+名前、リンク、テキストの順）から要素を特定し、自動待機付きで要素中心をクリックする。ロケータで特定できない場合のみfingerprint座標、次に記録座標へフォールバックする。承認対象化した操作、stale revision、許可外URLは続行しない。遷移操作は遷移先パスの一致（クエリ差は許容）を確認し、画面内操作（前後URLが同一）はURL照合を行わない。 |
| FR-RP-009 | ロケータ・座標いずれでも継続できない、または承認が必要でユーザー停止以外の場合、現在画面を維持して元の要求を通常のComputer Useへ1回だけ引き継ぐ。引き継ぎ後も許可ドメイン、重要操作承認、最大ステップ、Verifier、安全停止を適用する。遷移後URLが一致しない場合は、そのステップの記録到達URLへ進むことだけを目標にAIへ1手だけ限定引き継ぎし、到達を確認できたら次の記録ステップからリプレイを再開する。限定引き継ぎの上限は3回とし、AI補助は操作履歴へ保存しない。 |
| FR-RP-010 | UIは収集状態、日時、元要求、開始・終了URL、結果、操作数、リプレイ可否、各操作の内容と位置、およびリプレイ進捗を表示し、停止、単件削除、一括削除を提供する。 |
| FR-RP-011 | 操作履歴は初期設定で30日保持し、期限超過分を削除する。ローカルデモではサーバー側の非公開データファイルへ原子的に保存し、ソース管理対象外とする。 |
...
| タスクモード | `POST /api/chat`、`POST /api/realtime/message`、`POST /api/realtime/tool` の `mode=task` | 複合要求を計画、段階実行、検証する。省略時または `mode=normal` は通常モードを使用する。 |
...
| 操作履歴一覧・一括削除 | `GET/DELETE /api/run-history` | agent run履歴の一覧取得または一括削除を行う。 |
| 操作履歴詳細・単件削除 | `GET/DELETE /api/run-history/{runId}` | 操作内容、URL、位置を取得または単件削除する。 |
| 操作履歴リプレイ | `POST/DELETE /api/run-history/{runId}/replay` | リプレイを開始し、DELETEで実行中のリプレイを停止する。 |
...
| `MODEL_OPERATION_TIMEOUT` | タスク計画、次手計画、または検証の一次・フォールバックモデルが制限時間を超過 | 処理ログで失敗段階を確認し、再試行を提示する。 |
...
| `RUN_HISTORY_NOT_FOUND` | 指定した操作履歴が存在しない | 一覧を再取得する。 |
| `RUN_HISTORY_NOT_REPLAYABLE` | 文字入力など再現対象外の操作を含む | 理由を表示し、通常の要求実行を案内する。 |
| `REPLAY_ALREADY_RUNNING` | 別のリプレイが実行中 | 現在のリプレイ停止後に再試行する。 |
...
- 操作履歴の収集目的、保存内容、文字入力値を保存しないことを履歴UIで明示する。
...
- 重要操作の内容、承認・拒否・期限切れ
- プロファイル、興味、履歴収集設定の変更
- 操作履歴収集設定の変更、リプレイの開始・停止・照合失敗・AI引き継ぎ
- モデル、ブラウザ、音声、永続化の障害
...
| NFR-010 | リプレイ性能 | 同一環境で成功するリプレイはcomputer-useモデル呼び出しを0回とし、元runより短い時間で完了する。最初の保存操作は開始要求から平常時2秒以内に実行を開始する。 |
...
| リプレイ対象不一致 | 保存座標で続行せず、現在画面と失敗位置を保持して元の要求をComputer Useへ引き継ぐ。 |
| リプレイ停止 | 後続の保存操作とAI引き継ぎを実行せず、現在画面で手動操作へ戻す。 |
...
**Given** タスクモードが選択され、対象URLを表示したbrowser sessionが準備済みである
**When** ユーザーが、依存関係を持つ複数の包含条件、除外条件、数量条件または比較条件を含む要求を送信する
**Then** Goal Plannerは要求をUI非依存の構造化ゴール制約へ変換する。Next-Step Plannerは現在画面から実行可能な次の1サブタスクだけを生成し、Executorが実行し、独立したVerifierが事後条件と全ゴール制約を判定する。未観測の後続UIや複数カテゴリのUIが同時に表示されることを仮定しない。操作数または実行時間のチャンク境界では状態を保持して自動継続し、HTTPエラーや完了として扱わない。制約結果の欠落、証拠不足、条件違反、後続操作による選択解除がある場合は完了にせず、最新画面から次の1手を再計画する。承認後に再開した操作にも同じ検証を適用し、ゴール制約をすべて確認するまで完了を報告しない。

#### AC-012-A 代表シナリオ: IS見積もり

**Given** AC-012の条件を満たすbrowser sessionが準備済みである
**When** ユーザーが「ISモデルのFスポーツで、ボディカラーを白にして、メーカーオプションを全部付けた見積もりが見たい」と依頼する
**Then** Goal PlannerはIS、F SPORT、白系ボディカラー、選択可能なメーカーオプションの全選択、選択済み販売店オプション0件、見積金額の表示を独立した構造化ゴール制約として生成する。Next-Step Plannerは表示中の画面に応じて、モデル選択、グレード選択、カラー選択、オプション選択などから次の1目的だけを生成する。選択済み要約にF SPORTが明示されていれば元のグレード選択UIへ戻らず達成済みとして扱う。Executorは「販売店オプション」と表示された項目を選択せず、既に選択済みの場合は解除する。Verifierは各制約を画面上で確認し、販売店オプションの混入または0件である証拠の欠落がある場合は完了を拒否する。

### AC-013 操作履歴の収集制御

**Given** 操作履歴収集がONでAIのagent runが実行中である  
**When** ユーザーがUIから操作履歴収集をOFFにする  
**Then** 当該runの部分履歴が破棄され、以後のrunは保存されない。既存の完了履歴と興味推定用の行動履歴は維持される。再度ONにした場合は次のrunから保存される。

### AC-014 高速リプレイ

**Given** 文字入力を含まず完了した操作履歴が存在する  
**When** ユーザーが履歴一覧からリプレイを開始する  
**Then** 開始URLへ移動し、URL、操作内容、操作位置および対象要素を照合しながら保存順に操作する。成功経路ではcomputer-useモデルを呼び出さず、元runより短い時間で同じ終了URLへ到達する。

### AC-015 リプレイの安全停止と引き継ぎ

**Given** 保存後にページ構造または対象URLが変化している  
**When** リプレイ中の対象要素またはURL照合が失敗する  
**Then** 保存座標で後続操作を続けず、失敗位置と現在画面を保持し、元の要求を通常のComputer Useへ1回だけ引き継ぐ。ユーザーが停止した場合は引き継がない。文字入力を含むrunは開始前にリプレイ不可と表示する。
...
- agent run操作履歴、独立した収集設定、履歴一覧、高速リプレイ、安全なAI引き継ぎを実装する。
...
- 操作履歴の独立したON/OFF、途中OFF時の破棄、文字入力値の非保存、直接リプレイ、不一致時の安全なAI引き継ぎを確認している。
