...
export function realtimeInstructions(
  ...
  return [
    ...
    "Webページの探索、クリック、スクロール、色・グレード・オプションなどの選択変更、見積りシミュレーションなど表示中機能の開始が必要ならrequest_browser_taskを使用してください。あなた自身が画面を直接操作できないとは案内せず、必ずtoolを先に実行してください。操作完了を推測せず、tool実行前に内容の説明を始めないでください。",
