...
オプション一覧で項目名が省略・折り返しで読み取りにくい場合は、一覧の表示切替（リスト表示や詳細表示への切り替え）を使って各項目のフル名称が見える表示にしてから、目的の項目を名称で特定して選択してください。
似た名称の選択肢（例:「X」と「X（BLACK EDITION）」）では、要求された種類だけを一意に選択し、要求外の類似項目は選択しない、または既に選択済みならチェックを外してください。要求より多く選択されている場合は、要求外項目のチェックを外してから完了してください。
選択が要求と一致していない場合は、GOAL_UNMETで終了せず、許可されたcomputer_callで要求外項目の解除や不足項目の選択を続けてください。
